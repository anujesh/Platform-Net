...
namespace Platform.Id
{
        ...
        where TUser : IdentityUser
    {
        ...
        public List<TUser> GetUserByName(string userName)
        {
            ...
            return getUsers(commandText, parameters);
        }

        public TUser GetUserByEmail(string email)
        {
            string commandText = "Select * from Users where Email = @email";//@email1";
            Dictionary<string, object> parameters = new Dictionary<string, object>() { { "@email", email } };

            TUser user = getUser(commandText, parameters);

            return user;
        }


        private List<TUser> getUsers(string commandText, Dictionary<string, object> parameters)
        {
        ...
        private TUser getUser(string commandText, Dictionary<string, object> parameters)
        {
            TUser user = (TUser)Activator.CreateInstance(typeof(TUser));
        ...
        private static TUser mapUser(Dictionary<string, string> row)
        {
            TUser user = (TUser)Activator.CreateInstance(typeof(TUser));
            user.Id = row["Id"];
            user.UserName = row["UserName"];
        ...
        private static List<TUser> mapUsers(List<TUser> users, Dictionary<string, string> row)
        {
